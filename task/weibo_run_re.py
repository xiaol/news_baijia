...
def find_Index_similar_with_compare_news_2(training_data, data_to_classify):
    docs = {}
    docs.update(training_data)
    docs.update(data_to_classify)
    names = docs.keys()
    ...
    for i, name in enumerate(names):
        if name=="doc":
            ...
            for sims_elem in sims:
                if sims_elem[0]=="doc":
                    continue
                elif (sims_elem[1]>0.2 and sims_elem[1]<0.6) or (same_word[sims_elem[0]]>=keyword_num*0.1 and same_word[sims_elem[0]]<=keyword_num*0.3 and keyword_num>=10):
                    print "article_title,%s"%docs[name]
                    paragraphIndex_dict[sims_elem[0]] = { "similarity": sims_elem[1]
                                                         , "unit_vec" : unit_vec[sims_elem[0]]
                                                         , "keyword": keyword}
...
if __name__ == '__main__':
    ...
    # print gist
    gist = g().get_gist(r"2015年12月8日，北京，北京首次启动“空气重污染红色预警”，市民游客纷纷戴口罩出行。视觉中国供图，谢绝转载")
    # gist = fetch_gist_result(r"2015年12月8日，北京，北京首次启动“空气重污染红色预警”，市民游客纷纷戴口罩出行。视觉中国供图，谢绝转载")
