...
def newsFetch_channel(channelId, page=1, limit=50):
    ...
    if channelId >= 0:
        docs = conn['news_ver2']['NewsItems'].find(
            {"channel_id": str(channelId), "imgnum": {'$gt': 0}, 'create_time': {'$exists': True}}).sort("create_time",
...
def constructEvent(eventList):
        ...
        for eventElement in eventList:
            if eventElement['eventId'] == eventElement["source_url"]:
                ...
                subElement = {'sourceSitename': eventElement['start_title'], 'url': eventElement['source_url'],
                              'title': eventElement['title'], 'create_time': eventElement['create_time']}
                sublist.append(subElement)
            result_doc["create_time"] = eventElement['create_time']
