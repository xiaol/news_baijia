...
def createAlbum(user_id, album_title, album_des, album_img, album_news_count):
    ...
    for doc in docs:
        results_docs['albumId'] = str(doc['_id'])

    return results_docs


def updateAlbum(album_id,user_id, album_title, album_des, album_img, album_news_count):
    conn = DBStore._connect_news
    album_id = bson.objectid.ObjectId(album_id)
    db = conn["news_ver2"]["AlbumItems"]
    results_docs = {}
    docs = db.update({"album_id": album_id}).set({ "album_title": album_title, "album_des": album_des, "album_img": album_img,
...
def fetchAlbumList(user_id):
    ...
    for doc in docs:
        ...
        if "user_id" in doc.keys():
            results_docs['userId'] = doc['user_id']
        if "album_title" in doc.keys():
            results_docs['albumTitle'] = doc['album_title']
        if "album_des" in doc.keys():
            results_docs['albumDes'] = doc['album_des']
        if "album_img" in doc.keys():
            results_docs['albumImg'] = doc['album_img']
        if "album_news_count" in doc.keys():
            results_docs['albumNewsCount'] = doc['album_news_count']
        if "create_time" in doc.keys():
            results_docs['createTime'] = doc['create_time']
