...
########################################################################################
    def antonyms(self):
        ...
        cursor.execute("SELECT antonym_word, ref_id FROM cwn_antonym WHERE cwn_id ='"+selfId+"' UNION SELECT antonym_word, ref_id FROM cwn_antonymtmp WHERE cwn_tmpid ='"+selfId+"'")
        all_tuple = cursor.fetchall()
        #print all_tuple # (u'\u524d\u65b9', u'0100')
        if len(all_tuple) == 0:
            nyms = []
...
#########################################################################################            
    def hypernyms(self):
        ...
        ############# GET XXXnym/ref_id BY MATCHING CWN_ID #########
        cursor.execute("SELECT up_word, ref_id FROM cwn_up WHERE cwn_id ='"+selfId+"' UNION SELECT up_word, ref_id FROM cwn_uptmp WHERE cwn_tmpid ='"+selfId+"'")
        all_tuple = cursor.fetchall()
        #print all_tuple # (u'\u524d\u65b9', u'0100')
        if len(all_tuple) == 0:
            nyms = []
