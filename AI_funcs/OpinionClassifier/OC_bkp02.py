...
class NewsArticle:
    def __init__(self):
        ...
        result_doc = conn["news_ver2"]["googleNewsItem"].find({"eventId": {"$exists": 1}, "text": {"$exists":1}, 'createTime':{"$gte": '2015-07-26 06:00:00',"$lt": '2015-07-27 06:00:00'}, "eventId":{'$exists': True}, "duplicate_check":{'$exists': True}}).sort([("createTime", -1)]).skip(10).limit(10)
        self.sens_and_ids = {}
        sorted_docs = {}
...
class FrequentFeature:
    ...
    def compactify(self):
        opq = NewsArticle().get_article_sentences()
        print(opq)
        for k, v in opq.iteritems():
            ...
            for kk, vv in v.iteritems():
                ...
                for vvv in vv:
                    ...
                    vvv = vvv.decode('utf8')
                    print('Sentence : '+ vvv)
                    print('Sentence length: ' + str(len(vvv)))
                    jb = jieba.cut(vvv)
                    see = []
                    position = 0
                    for jjbb in jb:
                        ...
                        print(jux)
                        print(jjbb)
                        jjbb = json.dumps(jjbb, ensure_ascii=False)
                        see.append(jjbb)
        ...
        for me in men:
            ...
            for m in me:
                print(len(m[0]))
                m = json.dumps(m[0], ensure_ascii=False)
                print(m)
...
if __name__ == '__main__':
    ...
    #             print(see)
    xyz = FrequentFeature()
    # xyzzz = xyz.get_freq_feature_candidates()
    # print(xyzzz)
    xyz.compactify()
