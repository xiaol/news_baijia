...
def cut_with_stop(sentence,cut_all=False,HMM=True):
    ...
    for blk in blocks:
        ...
        if re_han.match(blk):
            for word in cut_block(blk):
                if word.encode(encoding='utf-8') not in stop_word:
                    yield word
        else:
            ...
            for x in tmp:
                if re_white.match(x):
                    continue
                if re_skip.match(x):
                    if x.encode(encoding='utf-8') not in stop_word:
                        yield x
                elif not cut_all:
                    for xx in x:
                        if xx.encode(encoding='utf-8') not in stop_word:
                            yield xx
                else:
...
@require_initialized
def load_stopdict(f=None):
    ''' Load personalized stop  dict to improve detect rate.
    Parameter:
        - f : A plain text file contains words and their ocurrences.
    '''
    global stop_word
    if not f:
        _curpath=os.path.normpath( os.path.join( os.getcwd(), os.path.dirname(__file__) ))
        f=findTheFilePath(_curpath,'../extra_dict/stop_words.txt')
        # f=os.path.abspath('/Users/galois/exercise/scrapy/news_spider_version2/news_spider_version2/analyzer/extra_dict/stop_words.txt')
...
def findTheFilePath(curpath,speciFile):
    ...
    finalPath=os.path.join(parentPath,speciFilePath)
    return finalPath
